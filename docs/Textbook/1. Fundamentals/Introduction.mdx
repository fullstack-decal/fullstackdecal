...
1. **Structure**: We first map out the structure of the website. We define the sections of the websites that we will
style and functionalize later. 
2. **Style**: Now that we know _where_ everything will go, we have to define how it will look. Any modern website
should have a well-defined design that is appealing to the user. 
3. **Functionality**: Not every website is functional, but for the ones that are, we have to exactly define and execute
the functions that give our website the desired functionality. 

Luckily for us, each of these three facets have tools that are made specifically to achieve this purpose in most efficient
and effective way possible. You will find that even as we move beyond these intitial tools into more complicated topics,
the workflow is the same, the tools just become more powerful.
