...
In `apod-server/index.js`, you can now set the value of `API_KEY` to your newly generated API key. You will need to use it for the first part of the homework when making requests to the NASA API. 
...
Before we start building anything, we have to ensure that all of the dependencies have been properly installed for the Express server:
...
**Hint:** You can access the response body of an API response using the `data` property
...
**Hint:** What parameters does the NASA API provide other than `date`? You can find this on the https://api.nasa.gov/ website -> "Browse APIs" -> "APOD". 
...
**Note:** The starter code for Task 3 is outdated, sorry! Implement it how it is described here in the spec. You may find it useful to change `index.html` so that the `img` element with id `q3-img` is instead a div, and style the div the same way as "q2-container" so it displays the images neatly. Additionally, the code in `/apod-frontend/index.js` should be changed to display all images for the month, similar to in question 2. If you get stuck, [here](https://codepen.io/michellewang5/pen/bGxjvYY) is what the starter code for Task 3 should look like.   
...
- an array of APOD objects, of the form below, where each element is in the same format as your response to Task 1
