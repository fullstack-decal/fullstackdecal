...
Elements you create are rendered as boxes on a page. Previously, we looked
at how we can position these boxes on the page, but you can actually do a
lot more with this. That's where the display property comes in.

Display properties describe the type of rendering box around an element. Some
of the different styles we can use include...
...
Inline elements are arranged one after the other. Like any othr style,
these elements are contained in boxes that are placed adjacent to one another.
A common use for inline elements is for buttons placed side-by-side.
...
This is an easy way to lay out your elements in a **grid**. A Grid places the elements
in a two-dimensional matrix. The vertical lines are columns and the horizontal lines are
rows. Using a Grid allows you to change the dimensions of the columns and rows, making it
a strong choice for placing elements. However, realize that the Grid doesn't have to be
a perfect matrix. You can merge columns at specific rows, wrap content, and give individual
items different widths to customize the layout.

#### Grid container

A Grid container is the element that contains the items of the Grid. We can create a
Grid container by specifying the display properties (see the example at the bottom of this section).

#### Grid lines

Grid lines form the grid layout that divides the container into rows and columns. They
can be referenced using an index that starts at 1 from the top left corner. This means
that the first row grid line is index 1 (not 0, unlike Python and Java arrays/matrices).

#### Grid cells

Grid cells are formed at the intersection of a row and column Grid line. They are bounded
by exactly 2 row lines and 2 column lines to form a box. This is where you can place
elements into the Grid. Grid cells are the smallest possible unit of space on the Grid

#### Grid area

Grid areas are formed by combining one or more adjacent Grid cells to form a larger space.
Therefore, Grid cells are also Grid areas.
...
positioning. The main purpose of a flex-box is to fit all elements within
the container by changing the flex dimensions. The flex container can shrink
and stretch as needed, making it very useful in responsive websites where element
sizes change.

A huge benefit of the Flex-box is that it is content aware. Different rows uniquely respond
to elements within based on available space. It is one-dimensional along rows, so it can
change item widths without notice of columns. Take advantage of this by avoiding specifying
a width when creating the CSS for a Flex-box. This removes the main flexible purpose of using
the Flex-box since items will no longer be dynamic.

The flex-grow propery specifies how items in the flex-box should take up available
space. The value defaults at 0, and it must be specified within a CSS class specific
to the element. This is similar to the flex-wrap property, which will wrap items To
the next line if there are too many elements present in the container.

You can use the justify-content property to align the elements contained within.
When items are inflexible, it helps with using available space in the flex-box to
align them across the container.
...
Here are some resources to learn more about the usage of the Flex-box display option:
...
specification that makes designs look cleaner and well aligned. You can
lay out items easily by placing them into Grids that encapsulate a large
number of components on your page.

#### Which is better if I'm concerned about the flow of my content?

Try Flex-box. Flex-box is useful because the container changes the
height and width of elements within, which makes it more responsive
to the page. However, if you're looking for something more rigid and
focuses more on the placement of elements, Grid would be better.

## Float

We won't go into float too much in this reading, but know that setting
float to either "left" or "right" shifts the element to the left or right,
respectively, of the container that it is encapsulated in. The rest of the
content will be wrapped around it.

Suppose we had an inline `<img>` nested within a `<p>`. Setting the image's float
property to "right" would place the image to the right of the container and
wrap the text above, to the left of, and below it.
...
- Cascade allows you to differentiate child properties from that of their parent.
...
Why is this important? Well suppose you are working on your styles for a component.
If your applied CSS styles don't seem to show up, take a dive into understanding
whether there are other conflicting rules with the same specificity. You might not
be expecting another rule to be overwriting what you wanted.
