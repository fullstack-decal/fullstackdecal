...
for each question. If you forgot how JavaScript functions work, refer back to the [JavaScript lesson](/docs/Lessons/JSIntro).
...
When the sidebar is open, have the button's text be `‹` (this is not an angle bracket!). Make sure to
...
containing the text that the user entered. After adding the TODO, clear the input. Validate that the todo task name is not empty before adding the list.
...
Make sure all your functionality works when viewing the html file before submitting. To submit, push the files to a Github repository and submit on Gradescope. If you feel comfortable with creating Github repositories and no longer want to create a new one for each assignment, you can zip up the individual files and use the Upload File option on Gradescope!
